<!-- Retrieved via Recall MCP (card 0158be25-928e-4834-9044-7d11f674df01, share 644fa8b1-e016-50e7-be7d-0f6ae2f13c70).
     Full ordered text from source gist: https://gist.github.com/chad/60ad3c201299dfe9d762a27834772f5f -->
...
These compose. A *directed* action can still be posted **in-channel** (`act-to= ` on a `TAGMSG #ops`) so it's assigned to one agent but everyone watches it happen. An **open direct action** (no `act-to`, DM target) is legal but pointless — there's exactly one other participant to claim it — so implementations may reject it as malformed.
