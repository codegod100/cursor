...
# Detect issue; exit 0 (success, skipped) when this build is not a new issue.
set +e
...
2. Run relevant tests or verification commands if they exist.
3. Use the **radicle** MCP tool \`create_patch\` to propose your changes on the \`rad\` remote:
...
DELEGATE="$REPO_ROOT/.cursor/skills/cursor-agent/scripts/delegate.sh"
if [[ ! -x "$DELEGATE" ]]; then
